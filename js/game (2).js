...
(() => {
  ...
  function movementBlocked(px, pz, ndx, ndz, dist, radius, baseY) {
    ...
    if (rayBlocked(px, pz, ndx, ndz, dist, radius, baseY, [HIGH_WALL_H])) return true; // 진짜 벽 → 무조건 차단
    ...
    if (groundY !== null && groundY - baseY > 0.02 && groundY - baseY <= STEP_MAX) return false; // 오를 수 있는 턱 → 통과
