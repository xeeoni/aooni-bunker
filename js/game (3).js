...
(() => {
  ...
  // ⚠ 버그: 생존자(스틱맨)가 걷는 동안 바닥을 뚫고 들어가 허리 위쪽만 보이는 현상.
  //   원인: idle/walk/run/jump 클립은 전부 아오오니 원본 리그(mixamorigHips 등)에서 뽑은
  //   것인데, 이 트랙 중 "mixamorigHips.position"에는 그 원본 리그의 골반(Hips) 높이가
  //   원래 단위 그대로(스케일 보정 전) 절대값으로 박혀 있다. 아오오니는 자기 자신의 raw
  //   높이 기준으로 정규화(SCALE = 1.8 / rawHeight)되므로 이 값이 우연히 잘 맞아떨어지지만,
  //   생존자는 완전히 다른 스틱맨 FBX를 "자기 자신의" raw 높이 기준으로 따로 정규화
  //   (SURVIVOR_SCALE = 1.7 / sRawHeight)한다. 두 스케일 비율이 다르기 때문에 같은 절대
  //   Hips.position 값이 생존자 리그에서는 실제 골반 높이보다 훨씬 아래(때로는 바닥 밑)를
  //   가리키게 되고, 매 프레임 mesh.position.y를 standY로 맞춰도(그룹 자체는 바닥 위에 있어도)
  //   그 안의 스켈레톤이 Hips 트랙 때문에 아래로 끌려 내려가 다리~골반이 바닥을 뚫는다.
  //   → 이 트랙의 "높이(Y)" 성분만 제거해서, 애니메이션이 캐릭터를 위아래로 잘못 끌고
  //   다니지 못하게 한다(좌우/앞뒤 흔들림은 자연스러움을 위해 남겨둔다). 그룹의 y좌표는
  //   여전히 update()에서 standY로 정확히 맞춰지므로 서 있는 높이는 지형 스냅 로직이 전담한다.
  function flattenHipsHeight(clip) {
    for (const track of clip.tracks) {
      if (/Hips\.position$/i.test(track.name)) {
        for (let i = 1; i < track.values.length; i += 3) track.values[i] = 0;
      }
    }
    return clip;
  }
  ...
  function rayBlocked(px, pz, ndx, ndz, dist, radius, baseY = floorY, heights = LOW_HIGH_HEIGHTS) {
  ...
  // 계단으로 오를 수 있는 높이(STEP_MAX=0.6) 바로 위 + 머리 근처. 두 높이를 따로/함께 검사할 때 사용.
  // ⚠ STEP_MAX 상수 자체는 아래(구역 "계단/경사로")에서 선언되지만, 이 값들은 함수 안에서만
  //   쓰이고 함수는 전부 animate() 루프가 돈 뒤(즉 스크립트 전체가 한 번 실행된 뒤)에만 호출되므로
  //   선언 순서와 무관하게 안전하다.
  const LOW_STEP_H = 0.65; // STEP_MAX(0.6)보다 살짝 높게 잡아 계단 챌판을 여유 있게 통과시킨다
  const HIGH_WALL_H = 1.5; // 머리 근처 — 진짜 벽/천장 높이 장애물 감지용
  const LOW_HIGH_HEIGHTS = [LOW_STEP_H, HIGH_WALL_H];

  // ⚠ 이 벙커 모델은 계단이 "바닥(HouseFloor)"이 아니라 "벽(HouseWalls)" 메시 안에 함께
  //   붙어 있다(내보내기 전에 벽+계단이 한 덩어리로 합쳐진 것으로 보인다). 그래서 계단의
  //   챌판(수직면)에 rayBlocked가 맞으면 무조건 "벽"으로 처리돼 아무리 STEP_MAX를 키워도
  //   근본적으로 못 올라가는 경우가 생긴다.
  //   ⚠⚠ 지난 수정에서는 "막혔으면 그 앞에 밟을 바닥이 있는지"만 보고 통과 여부를 정했는데,
  //   이러면 "머리 높이까지 막힌 진짜 벽"도 벽 너머에 바닥(다른 방, 심지어 맵 밖 허공)만
  //   있으면 그냥 뚫고 지나가 버렸다(신고해주신 "맵이 다 뚫린다" 증상의 원인).
  //   → 낮은 높이(STEP_MAX 근처)와 머리 높이를 따로따로 검사한다.
  //   · 머리 높이까지 막혀 있으면 무조건 진짜 벽으로 보고 절대 통과시키지 않는다.
  //   · 낮은 높이만 막혀 있을 때만 "계단 챌판일 수 있다"고 보고, 그 앞에 STEP_MAX 이내로
  //     오를 수 있는 바닥이 있는지 추가로 확인해서 있을 때만 통과시킨다.
  // ⚠ 계단을 못 올라가는 버그의 원인: 이 벙커 모델은 계단이 위로 갈수록 그 위 다음
  //   층/계단참의 구조물(천장·다음 단)도 같이 촘촘히 붙어 있다. 예전 코드는 HIGH_WALL_H(머리
  //   높이) 검사를 항상 "지금 서 있는 높이(baseY)" 기준으로만 했는데, 계단을 오르려는 바로
  //   그 순간엔 발밑(baseY)이 아직 낮은 단 위라서, 딱 그 낮은 위치에서 잰 "머리 높이"가
  //   계단 자체(한두 칸 위의 디딤판/챌판)에 걸려버린다 — 실제로는 한 칸 올라서면 머리 위가
  //   훤히 뚫려 있는데도 "진짜 벽"으로 오판해 계단 초입에서 영원히 막히는 것이다.
  //   → 먼저 앞쪽에 오를 수 있는 턱(계단 한 칸)이 있는지 확인하고, 있다면 "그 턱 위에
  //   올라섰을 때의 높이"를 기준으로 머리 높이 검사를 다시 한다. 진짜 막다른 벽이면 그
  //   기준으로도 여전히 막히고, 계단이면 뚫린다.
  function movementBlocked(px, pz, ndx, ndz, dist, radius, baseY) {
    if (dist <= 1e-6) return false;
    const aheadX = px + ndx * dist, aheadZ = pz + ndz * dist;
    const groundY = sampleGroundY(aheadX, aheadZ, baseY);
    const canStep = groundY !== null && groundY - baseY > 0.02 && groundY - baseY <= STEP_MAX;
    const headBaseY = canStep ? Math.max(groundY, baseY) : baseY; // 계단이면 오른 뒤 높이 기준으로 머리 검사
    if (rayBlocked(px, pz, ndx, ndz, dist, radius, headBaseY, [HIGH_WALL_H])) return true; // 진짜 벽 → 무조건 차단
    if (!rayBlocked(px, pz, ndx, ndz, dist, radius, baseY, [LOW_STEP_H])) return false; // 아무것도 안 막힘
    if (canStep) return false; // 오를 수 있는 턱 → 통과
    return true; // 낮지만 오를 수 없는 턱(내려가는 계단 옆판 등)
  }

  function tryMove(dx, dz) {
    const pos = playerRig.position;
    const distX = Math.abs(dx), distZ = Math.abs(dz);
    const blockedX = distX > 1e-6 && movementBlocked(pos.x, pos.z, Math.sign(dx), 0, distX, PLAYER_RADIUS, pos.y);
    const blockedZ = distZ > 1e-6 && movementBlocked(pos.x, pos.z, 0, Math.sign(dz), distZ, PLAYER_RADIUS, pos.y);
  ...
  // ⚠ 예전에는 위에서 아래로 쐈을 때 "가장 먼저(가장 위에서) 맞은" 위쪽 면을 무조건 바닥으로
  //   썼다. 그런데 문틀/난간/선반처럼 진짜 바닥보다 훨씬 높은 곳에 있는 (위를 향한) 작은
  //   면이 그 지점 위를 지나가면, 실제 바닥이 더 아래에 있어도 그 높은 면이 먼저 걸려서
  //   "바닥"으로 오인됐다 → 생존자/아오오니가 벽·문틀 꼭대기 위에 붕 뜨는 원인.
  //   → 모든 위쪽-면 히트를 다 모은 뒤, "지금 서 있(었)던 높이(preferY)"에 가장 가까운
  //   히트를 고른다. 계단을 오를 때는 preferY(현재 높이)와 다음 디딤판이 STEP_MAX 이내로
  //   가까우므로 자연스럽게 그 디딤판이 선택되고, 훨씬 높은 난간/선반은 무시된다.
  function rawGroundHit(x, z, aroundY, preferY) {
    ...
    for (const hit of hits) {
      ...
      if (worldNormal.y <= 0.5) continue; // 충분히 "위쪽"을 향한 면만 바닥 후보로 인정 (난간/벽면 제외)
      const diff = Math.abs(hit.point.y - preferY);
      if (diff < bestDiff) { bestDiff = diff; best = hit.point.y; }
    }
    return best;
  }
  // preferY: "이 높이에 가장 가까운 바닥"을 찾으라는 기준점. 생략하면 aroundY와 동일
  // (제자리에서 발밑 높이를 갱신하는 일반적인 경우). 스폰 위치 탐색처럼 aroundY를 일부러
  // 높게 잡아 레이 시작점만 올리고 싶을 때는 preferY를 floorY처럼 따로 넘긴다.
  function sampleGroundY(x, z, aroundY, preferY = aroundY) {
    if (!groundMeshes.length) return null;
    const readings = [];
    for (const [ox, oz] of GROUND_SAMPLE_OFFSETS) {
      const y = rawGroundHit(x + ox, z + oz, aroundY, preferY);
  ...
  class Survivor {
    ...
    update(dt, playerPos) {
      ...
      if (!moving) {
        ...
        // 사방이 막힌 것으로 오판되지 않는다.
        const blocked = stepDist > 1e-6 && movementBlocked(this.mesh.position.x, this.mesh.position.z, step.x / stepDist, step.z / stepDist, stepDist, 0.3, this.standY);
        const nx = this.mesh.position.x + step.x, nz = this.mesh.position.z + step.z;
        if (!blocked) {
          // 혹시 모를 충돌 판정 사각지대(모델 이음매, 얇은 지오메트리 등)에 대비한 안전장치 —
          // 생존자가 어떤 경우에도 벙커 바깥(레벨 경계 밖)으로는 못 나가도록 한 번 더 clamp.
          this.mesh.position.x = THREE.MathUtils.clamp(nx, levelBounds.min.x + 0.2, levelBounds.max.x - 0.2);
          this.mesh.position.z = THREE.MathUtils.clamp(nz, levelBounds.min.z + 0.2, levelBounds.max.z - 0.2);
        }
    ...
    try {
      ...
      bunkerObj.traverse(child => {
        if (child.isMesh) {
          ...
          // ⚠ 이 모델은 계단이 "바닥(HouseFloor)"이 아니라 "벽(HouseWalls)" 메시 안에 함께
          //   붙어 있어서, 벽을 발밑 판정 대상에서 완전히 빼면 계단 자체를 밟을 수 없게 된다
          //   (실제로 캐릭터가 계단 속으로 가라앉는 증상으로 나타났다). 그래서 벽도 다시 포함하되,
          //   난간/문틀처럼 진짜 바닥보다 훨씬 높은 곳의 "위쪽 면"을 잘못 고르지 않도록
          //   sampleGroundY() 쪽에서 "현재 높이에 가장 가까운 면"을 고르도록 바꿨다(위 참고).
          if (!isCeil) groundMeshes.push(child); // 계단/경사로 포함, 천장만 제외
      ...
      // ⚠ preferY로 floorY(실제 1층 바닥 높이)를 넘긴다 — 그냥 aroundY(floorY+3, 레이 시작점을
      //   높이려고 임의로 띄운 값)를 기준으로 "가장 가까운 면"을 고르면, 진짜 바닥보다 문틀/난간
      //   같은 높은 면이 3m에 더 가까워서 잘못 선택될 수 있기 때문이다.
      function findWalkablePoint(nearX, nearZ) {
        const guess = pickPointOnFloor(nearX, nearZ);
        if (guess && sampleGroundY(guess.x, guess.z, floorY + 3, floorY) !== null) return guess;
        for (let attempt = 0; attempt < 40 && floorBoxes.length; attempt++) {
          ...
          const gy = sampleGroundY(tx, tz, floorY + 3, floorY);
      ...
      if (floorBoxes.length) {
        ...
        if (sampleGroundY(cx, cz, floorY + 3, floorY) !== null) floorY = sampleGroundY(cx, cz, floorY + 3, floorY);
      } else {
      ...
      for (let i = 0; i < 24; i++) {
        if (floorBoxes.length) {
          ...
          const gy = sampleGroundY(jx, jz, floorY + 3, floorY);
      ...
      if (walkObj.animations[0]) {
        flattenHipsHeight(walkObj.animations[0]);
        charAnims.walk = { clip: walkObj.animations[0], action: charMixer.clipAction(walkObj.animations[0]) };
      }

      setLoading(88, '애니메이션을 불러오는 중... (질주)');
      const runObj = await loadFBX(ASSET.animRun);
      if (runObj.animations[0]) {
        flattenHipsHeight(runObj.animations[0]);
        charAnims.run = { clip: runObj.animations[0], action: charMixer.clipAction(runObj.animations[0]) };
      }

      setLoading(95, '애니메이션을 불러오는 중... (점프)');
      const jumpObj = await loadFBX(ASSET.animJump);
      if (jumpObj.animations[0]) {
        flattenHipsHeight(jumpObj.animations[0]);
