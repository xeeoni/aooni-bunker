...
(() => {
  ...
  function rayBlocked(px, pz, ndx, ndz, dist, radius) {
    ...
    for (const off of offsets) {
      ...
      for (const h of heights) {
        moveRay.set(new THREE.Vector3(ox, floorY + h, oz), dir);
        moveRay.near = 0;
  ...
  function tryMove(dx, dz) {
    ...
    const blockedX = distX > 1e-6 && rayBlocked(pos.x, pos.z, Math.sign(dx), 0, distX, PLAYER_RADIUS);
    const blockedZ = distZ > 1e-6 && rayBlocked(pos.x, pos.z, 0, Math.sign(dz), distZ, PLAYER_RADIUS);
  ...
  class Survivor {
    ...
    update(dt, playerPos) {
      ...
      if (toTarget.length() < 0.4) {
        ...
        const blocked = stepDist > 1e-6 && rayBlocked(this.mesh.position.x, this.mesh.position.z, step.x / stepDist, step.z / stepDist, stepDist, 0.3);
