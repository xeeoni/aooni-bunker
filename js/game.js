...
(() => {
  ...
  //   → rayBlocked가 막혔다고 나와도 즉시 포기하지 않고, "그 방향으로 이동했을 때 실제로
  //   밟을 수 있는 바닥이 있는지"(sampleGroundY, STEP_MAX 이내)를 한 번 더 확인한다.
  //   있다면 그건 진짜 벽이 아니라 계단 같은 "밟고 오를 수 있는 턱"이므로 통과시킨다.
  function movementBlocked(px, pz, ndx, ndz, dist, radius, baseY) {
    if (dist <= 1e-6) return false;
    if (!rayBlocked(px, pz, ndx, ndz, dist, radius, baseY)) return false;
    const aheadX = px + ndx * dist, aheadZ = pz + ndz * dist;
    const groundY = sampleGroundY(aheadX, aheadZ, baseY);
    if (groundY !== null && Math.abs(groundY - baseY) <= STEP_MAX) return false; // 오를 수 있는 턱 → 통과
    return true; // 진짜 벽(또는 오를 수 없을 만큼 높은 턱)
  }
