...
(() => {
  ...
  // 즉시(또는 부드럽게) 그 높이로 스냅해서 "타고 올라가는" 느낌을 준다.
  // 높이차가 너무 크면(진짜 수직 벽/턱) 원래 높이를 유지해서 뚫고 올라가지 않는다.
  const groundRay = new THREE.Raycaster();
  const STEP_MAX = 0.6;      // 한 번에 오를 수 있는 최대 높이차 (계단 한두 칸 정도)
  ...
  function sampleGroundY(x, z, aroundY) {
    ...
    groundRay.set(new THREE.Vector3(x, aroundY + GROUND_PROBE_UP, z), new THREE.Vector3(0, -1, 0));
    groundRay.near = 0;
