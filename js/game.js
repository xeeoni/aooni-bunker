    ...
    try {
      ...
      bunkerObj.traverse(child => {
        if (child.isMesh) {
          ...
          if (isFloor) floorBounds.union(b);
        }
      });
      ...
      // 스폰 위치는 "걸어다닐 수 있는 바닥(floorBounds)" 기준으로 잡는다 (맵 안에서 시작).
      const cx = (floorBounds.min.x + floorBounds.max.x) / 2;
      const cz = (floorBounds.min.z + floorBounds.max.z) / 2;
